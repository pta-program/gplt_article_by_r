...
# 按题目约束遍历当前数据，更新中间状态。
    for (case in seq_len(t)) {
        ...
        for (round in seq_len(20L)) {
            pts <- matrix(v[seq.int(p, p + 39L)], nrow = 20L,
                ncol = 2L, byrow = TRUE)
            p <- p + 40L
            da <- sqrt((pts[, 1] - xa)^2 + (pts[, 2] - ya)^2)
            db <- sqrt((pts[, 1] - xb)^2 + (pts[, 2] - yb)^2)
            ia <- which(da <= db)
            ib <- which(da > db)
            tour <- function(ids, sx, sy) {
                curx <- sx
                cury <- sy
                dist <- 0
                order <- integer()
                while (length(ids)) {
                  z <- ids[which.min((pts[ids, 1] - curx)^2 +
                    (pts[ids, 2] - cury)^2)]
                  dist <- dist + sqrt((pts[z, 1] - curx)^2 +
                    (pts[z, 2] - cury)^2)
                  curx <- pts[z, 1]
                  cury <- pts[z, 2]
                  order <- c(order, z - 1L)
                ...
                list(dist = dist, order = order, x = curx, y = cury)
            }
            ta <- tour(ia, xa, ya)
            tb <- tour(ib, xb, yb)
            tm <- max(ta$dist, tb$dist)/2
            if (total + tm > 120 + 1e-09)
                break
            total <- total + tm
            rounds[[length(rounds) + 1L]] <- list(ta$order, tb$order)
            xa <- ta$x
            ya <- ta$y
            xb <- tb$x
            yb <- tb$y
        }
        out <- c(out, as.character(length(rounds)))
        for (z in rounds) {
            out <- c(out, paste(length(z[[1]]), length(z[[2]])),
                paste(z[[1]], collapse = " "), paste(z[[2]],
                  collapse = " "))
        }
    }
# 严格按照题目规定的输出格式输出答案。
